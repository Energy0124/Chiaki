...
namespace PixivCS
{
    ...
    public class PixivBaseAPI
    {
        ...
        //用户名和密码登录
        public async Task<JsonDocument> Auth(string Username, string Password)
        {
            ...
            var jsonDocument = JsonDocument.Parse(await GetResponseString(res));
            var resJSON = jsonDocument.RootElement;
            AccessToken = resJSON.GetProperty("response").GetProperty("access_token").GetString();
            UserID = resJSON.GetProperty("response").GetProperty("user").GetProperty("id").GetString();
            RefreshToken = resJSON.GetProperty("response").GetProperty("refresh_token").GetString();
            if (RefreshInterval > 0) refreshTimer.Start();
            return jsonDocument;
        }

        //RefreshToken登录
        public async Task<JsonDocument> Auth(string RefreshToken)
        {
            string url = "https://oauth.secure.pixiv.net/auth/token";
            Dictionary<string, string> headers = new Dictionary<string, string>
            {
            ...
            AccessToken = resJSON.GetProperty("response").GetProperty("access_token").GetString();
            UserID = resJSON.GetProperty("response").GetProperty("user").GetProperty("id").GetString();
            this.RefreshToken = resJSON.GetProperty("response").GetProperty("refresh_token").GetString();
            if (RefreshInterval > 0) refreshTimer.Start();
            return jsonDocument;
        }
    }
